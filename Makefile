...
alice:
	export WASM_BUILD_TOOLCHAIN=`cat rust-toolchain`
	cargo run --release -- purge-chain --base-path /tmp/alice --chain local
	cargo run --release -- --base-path /tmp/alice \
		--chain local \
		--alice \
		--port 30333 \
		--ws-port 9945 \
...
bob:
	export WASM_BUILD_TOOLCHAIN=`cat rust-toolchain`
	cargo run --release -- purge-chain --base-path /tmp/bob --chain local
	cargo run --release -- --base-path /tmp/bob \
		--chain local \
		--bob \
		--port 30334 \
		--ws-port 9946 \
